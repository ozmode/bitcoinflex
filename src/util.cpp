...
const char * const BITCOIN_CONF_FILENAME = "bitcoinflex.conf";
const char * const BITCOIN_PID_FILENAME = "bitcoinflexd.pid";
...
#else
    const char* pszModule = "bitcoinflex";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinFlex";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitcoinFlex";
#else
    // Unix
    return pathRet / ".bitcoinflex";
#endif
#endif
}
